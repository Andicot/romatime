...
function eseguiAzioniResp(resp) {
    ...
    if (typeof resp.mostraOggetto !== 'undefined') {
        $("#" + resp.mostraOggetto).show();
    }
    ...
    if (typeof resp.oggettoReload !== 'undefined' && typeof resp.html !== 'undefined') {
        if (resp.oggettoReload !== '') {
            ...
            if (oggettoReload.length === 0) {
                console.log('L\'oggetto con ID=' + '#' + resp.oggettoReload + ' non esiste');
            } else {
    ...
    if (typeof resp.oggettoReplace !== 'undefined' && typeof resp.html !== 'undefined') {
        if (resp.oggettoReplace !== '') {
            ...
            if (oggettoReplace.length === 0) {
                console.log('L\'oggetto con ID=' + '#' + resp.oggettoReplace + ' non esiste');
            } else {
...
function formSubmitAndDelete(idPulsanteSalva) {
    //method: method chiamata ajax

    if (idPulsanteSalva === undefined) {
        idPulsanteSalva = '#submit';
    }

    console.log('formSubmitAndDelete');
    $(idPulsanteSalva).click(function (e) {
        e.preventDefault();
        formSubmit(this);
    });
        ...
        $.ajax('/segnala-problema/create',
            {
                ...
                success: function (resp) {
                    var target = '#kt_modal';
                    var url = '/segnala-problema/create';
...
function select2Citta(campo) {
    ...
    var campoCap = 'cap';
    if (campo.includes('_')) {
        //Determina nome campo cp
        const underscoreIndex = campo.indexOf('_');
        const wordAfterUnderscore = campo.substring(underscoreIndex + 1);
        campoCap = 'cap_' + wordAfterUnderscore;
    }
    ...
    $('#' + campo).select2({
        ...
        // Access to full data
        $("#" + campoCap).val(e.params.data.cap);
    });
}
...
function disableSubmitButton() {
    ...
    if (submitButton.disabled) {
        return false; // Evita il submit se il pulsante è già disabilitato
    }
