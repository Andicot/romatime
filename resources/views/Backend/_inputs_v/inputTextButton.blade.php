<div class=" mb-6" id="div_{{$campo}}">
    ...
    <div class="input-group mb-3">
        <input type="text" id="{{$campo}}" name="{{$campo}}" class="form-control form-control-solid {{$classe??''}}" placeholder="{{$placeholder??''}}" value="{{ old($campo,$record->$campo) }}"
               data-required="{{$required??''}}"
               @if($required??false) required @endif
               autocomplete="{{$autocomplete??''}}"
        >
        <button class="btn {{$classeButton??'btn-primary'}}" type="button" id="button-{{$campo}}">{{$labelButton??'Ok'}}</button>
    </div>
    @if($help??false)
